...
| `test_dependencies.tf` | A dedicated, throwaway resource group + DNS zone this harness owns outright - never a shared/production resource group. Its name is suffixed with `var.pr_number` so concurrently open PRs never collide. |
| `variables.tf` | `env`, `location` (defaults to `canadacentral`), `tags`, `pr_number` (defaults to `"manual"`), and `public_dns_records` (typed `any`, passed straight through to the module). |
| `config/public_dns_records.tfvars` | One representative real-usage fixture: a single map entry exercising the A-record common path. |
...
cd test/live
terraform init
terraform plan  -var-file=config/public_dns_records.tfvars
terraform apply -var-file=config/public_dns_records.tfvars
```

Confirm only the live-test resource group, DNS zone, and `module.public_dns_records`
are planned/applied, then tear it down:

```bash
terraform destroy -var-file=config/public_dns_records.tfvars
```
...
terraform apply -var-file=config/public_dns_records.tfvars -var="pr_number=<pr-number>"

# 2. PR plan (and, in CI, apply), from A, against the same state file.
cd A/test/live
terraform init -backend-config="path=$STATE"
terraform plan -var-file=config/public_dns_records.tfvars -var="pr_number=<pr-number>"

# 3. Always tear down from A once the run finishes (`if: always()` in CI).
terraform destroy -var-file=config/public_dns_records.tfvars -var="pr_number=<pr-number>"
```
